...
for i,learning_rate in enumerate(learningRates):
    ...
    for epoch in range(epochs):
        ...
        if(total < min_loss):
            ...
            torch.save(model.state_dict(),"best_model_" + str(learning_rate) + "_.pt")
...
plt.plot(np.array(learningRates),testError.numpy(), label = "train cost")
plt.plot(np.array(learningRates),validationError.numpy(), label = "validation cost")
plt.legend()
plt.show()

bestLoss = validationError[0]
for i in range(len(validationError)):
    if(validationError[i] < bestLoss):
        bestLearningRate = (i+1)*0.001
        bestLoss = validationError[i]

model_best = LR(2, 1)
model_best.load_state_dict(torch.load("best_model_" + str(bestLearningRate) + "_.pt"))
